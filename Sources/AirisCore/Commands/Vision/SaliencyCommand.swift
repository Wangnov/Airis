...
struct SaliencyCommand: AsyncParsableCommand {
    ...
    func run() async throws {
        ...
        // Parse saliency type
        let saliencyType: VisionService.SaliencyType = switch type.lowercased() {
        case "attention":
            .attention
        case "objectness":
            .objectness
    ...
    private func saveHeatmap(result: VisionService.SaliencyResult, to outputPath: String) throws {
        ...
        private static func testSaliencyResult(type: VisionService.SaliencyType) -> VisionService.SaliencyResult {
            ...
            let bounds: [CGRect] = if ProcessInfo.processInfo.environment["AIRIS_TEST_SALIENCY_EMPTY"] == "1" {
                []
            } else if type == .objectness {
                [CGRect(x: 0.1, y: 0.2, width: 0.3, height: 0.4), CGRect(x: 0.6, y: 0.5, width: 0.2, height: 0.2)]
            } else {
